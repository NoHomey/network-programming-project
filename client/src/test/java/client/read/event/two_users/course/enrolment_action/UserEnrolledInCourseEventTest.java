...
public class UserEnrolledInCourseEventTest {
    UserEnrolledInCourseEvent event;

    @Before
    public void initialize() {
        event = new UserEnrolledInCourseEvent(109, 6507, "cohort", 49);
    }

    @Test
    public void testFromDescription() {
        try {
            UserEnrolledInCourseEvent event = UserEnrolledInCourseEvent.fromDescription("The user with id '109' enrolled the user with id '6507' using the enrolment method 'cohort' in the course with id '49'.");
            assertEquals("The user with id '109' enrolled the user with id '6507' using the enrolment method 'cohort' in the course with id '49'.", event.valueForDescription());
    ...
    public void testValueForEventName() {
        assertEquals("User enrolled in course", event.valueForEventName());
    }
