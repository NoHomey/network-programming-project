...
public class LogReportViewedEventTest {
    LogReportViewedEvent event;

    @Before
    public void initialize() {
        event = new LogReportViewedEvent(2, 49);
    }

    @Test
    public void testFromDescription() {
        try {
            LogReportViewedEvent event = LogReportViewedEvent.fromDescription("The user with id '2' viewed the log report for the course with id '49'.");
            assertEquals("The user with id '2' viewed the log report for the course with id '49'.", event.valueForDescription());
    ...
    public void testValueForComponent() {
        assertEquals("Logs", event.valueForComponent());
    }

    @Test
    public void testValueForEventName() {
        assertEquals("Log report viewed", event.valueForEventName());
    }
